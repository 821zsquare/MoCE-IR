...
try:
    from net.conv_experts import HeterogeneousConvExpert
except ImportError:
    from conv_experts import HeterogeneousConvExpert
...
class AdapterLayer(nn.Module):
    def __init__(self, 
        ...
        expert_types = ["contmix", "fdconv", "pelk", "local"]
        self.expert_names = [expert_types[idx % len(expert_types)] for idx in range(num_experts)]
        kernel_by_type = {
            "contmix": 7,
            "fdconv": 3,
            "pelk": 31,
        ...
        self.experts = nn.ModuleList([
            HeterogeneousConvExpert(
                dim=dim,
                rank=rank,
                expert_type=expert_name,
                depth=stage_depth,
                kernel_size=kernel_by_type[expert_name],
                bias=False,
            )
    ...
    def forward(self, x, scale_context, shared):
        gates, top_k_indices, top_k_values, aux_loss = self.routing(x, scale_context)
        self.loss = aux_loss
...
class RoutingFunction(nn.Module):
    def __init__(self, dim, router_dim, num_experts, k):
        ...
        self.scale_gate = nn.Linear(router_dim, num_experts, bias=False)
    ...
    def forward(self, x, scale_context):
        logits = self.gate(x) + self.scale_gate(scale_context)
        if self.training:
        ...
        gating_scores = routing_logits.softmax(dim=-1)
        top_k_values, top_k_indices = torch.topk(gating_scores, self.k, dim=-1)

        # Final auxiliary loss
        if self.training:
            loss_load = self.load_loss(logits, routing_logits, self.noise_std)
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer,
                 rank=None, num_experts=None, top_k=None, stage_depth=None, router_dim:int=128):
        super().__init__()
        ...
        self.adapter = AdapterLayer(
            dim, rank,
            top_k=top_k, num_experts=num_experts, expert_layer=expert_layer, router_dim=router_dim,
    ...
    def forward(self, x, scale_context=None):
        ...
        x_a = self.proj[1](x)
        x_s = self.shared(x_s)
        x_a = self.adapter(x_a, scale_context, x_s)
...
class DecoderResidualGroup(nn.Module):
    def __init__(self, 
                 dim: int, num_heads: List[int], num_blocks: int, ffn_expansion: int, LayerNorm_type: str, bias: bool,
                 rank=None, num_experts=None, expert_layer=None, top_k=None, stage_depth=None, router_dim:int=128):
        super().__init__()
        ...
        for i in range(num_blocks):
            self.layers.append(
                DecoderBlock(
                    ...
                    stage_depth=stage_depth, router_dim=router_dim
                )
            )
    ...
    def forward(self, x, scale_context=None):
        ...
        self.loss = 0
        while i < len(self.layers):
            x , loss = self.layers[i](x, scale_context)
            self.loss += loss
            i += 1
        return x  
...
class ScaleGatedMSDR(nn.Module):
    def __init__(self, in_dims, router_dim, num_decoder_stages, num_degradation_attrs=6):
        super().__init__()
        ...
        self.stage_weight_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(router_dim * self.num_scales, router_dim),
                nn.GELU(),
                nn.Linear(router_dim, self.num_scales),
            )
            for _ in range(num_decoder_stages)
        ])
        self.degradation_head = nn.Sequential(
            nn.Linear(router_dim, router_dim),
            nn.GELU(),
            nn.Linear(router_dim, num_degradation_attrs),
        )

    def forward(self, features):
        tokens = [tokenizer(feat) for tokenizer, feat in zip(self.tokenizers, features)]
        token_stack = torch.stack(tokens, dim=1)
        token_cat = torch.cat(tokens, dim=-1)
        contexts = []
        weights = []
        for head in self.stage_weight_heads:
            alpha = head(token_cat).softmax(dim=-1)
            context = (alpha.unsqueeze(-1) * token_stack).sum(dim=1)
            contexts.append(context)
            weights.append(alpha)
        global_context = token_stack.mean(dim=1)
        degradation_logits = self.degradation_head(global_context)
        return contexts, degradation_logits, weights


##########################################################################
##
class MSDRMoEIR(nn.Module):
    def __init__(self,
                inp_channels=3, 
                out_channels=3, 
                dim = 32,
                ...
                router_dim=None,
                num_degradation_attrs=6,
                ):
        super(MSDRMoEIR, self).__init__()
        ...
        router_dim = router_dim or dims[-1]
        self.msdr = ScaleGatedMSDR(
            encoder_dims,
            router_dim=router_dim,
            num_decoder_stages=levels-1,
            num_degradation_attrs=num_degradation_attrs,
        )
        self.degradation_logits = None
        ...
        for i in range(levels-1):
            self.dec.append(nn.ModuleList([
                ...
                DecoderResidualGroup(
                    ...
                    num_heads=heads[i+1],
                    ffn_expansion=ffn_expansion_factor, 
                    LayerNorm_type=LayerNorm_type, bias=bias, expert_layer=expert_layer, router_dim=router_dim,
    ...
    def forward(self, x, labels=None):
        ...
        scale_contexts, self.degradation_logits, self.scale_weights = self.msdr(enc_feats + [feats])
                
        for i, (upsample, fusion, block) in enumerate(self.dec):
            feats = upsample(feats)
            feats = fusion(torch.cat([feats, enc_feats.pop()], dim=1))
            feats = block(feats, scale_contexts[i])
...
if __name__ == "__main__":
    # test
    model = MSDRMoEIR(rank=2, num_blocks=[4,6,6,8], num_dec_blocks=[2,4,4], levels=4, dim=48, num_refinement_blocks=4, 
                   stage_depth=[1,1,1],
